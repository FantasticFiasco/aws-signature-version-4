...
This project extends the class `HttpClient` by providing additional overloads to `DeleteAsync`, `GetAsync`, `PatchAsync`, `PostAsync`, `PutAsync`, `SendAsync`, and the new synchronous addition to .NET 5, `Send`. These overloads accept the following additional arguments.
